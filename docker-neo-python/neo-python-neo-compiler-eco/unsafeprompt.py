...
#TODO NEOCOMPILER imports for time and codecs, used on wait_tx and commands
import time
import codecs
#import resource
#import logzero
...
class PromptInterface(object):
    ...
    def do_create(self, arguments):
        ...
        if item and item == 'wallet':
            ...
            if path:
                ...
                password_key = to_aes_key(to_aes_key("coz"))
    ...
    def test_invoke_contract(self, args, onlyinvoke = False):
        if not self.Wallet:
            print("Please open a wallet")
            return
    ...
    def wait_for_tx(self, tx, max_seconds=150):
        ...
        while not foundtx and sec_passed < max_seconds:
            ...
            #time.sleep(3)
            sec_passed += 5
            while(timenow - timebase < 5):
                timenow = time.time()

            #if timenow - timebase > 1: #wait 3 seconds
            #    timebase = timenow #TODO
        if foundtx:
            return True
        else:
            ...
            try:
                ...
                if command is not None and len(command) > 0:
                    ...
                    if command == 'quit' or command == 'exit':
                        time.sleep(2) # TODO NeoCompiler.io - -consolidate chain
                        self.quit()
                    elif command == 'help':
                        self.help()
                    elif command == 'create':
