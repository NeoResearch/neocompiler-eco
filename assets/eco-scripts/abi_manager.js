...
function updateAllABIDependencies(jsonABI) {
    ...
    if (jsonABI.methods) {
        ...
        var i = 0;
        for (i = 0; i < jsonABI["methods"].length; i++)
            if (jsonABI["methods"][i]["name"] == "Main") {
        ...
        var paramhex = "";
        var paramnames = "";
        for (j = 0; j < jsonABI["methods"][i]["parameters"].length; j++) {
            var phex = getHexForType(jsonABI["methods"][i]["parameters"][j]["type"]);
            console.log("parameter[" + j + "]: " + jsonABI["methods"][i]["parameters"][j]["type"] + " -> hex(" + phex + ")");
            paramhex += phex;
            paramnames += jsonABI["methods"][i]["parameters"][j]["type"] + "\t";
        }
